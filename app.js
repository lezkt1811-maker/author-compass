...
function attachLibraryListeners() {
  ...
  if (form) form.onsubmit = (e) => {
    ...
    if (!title) return;
    setData((d) => {
      const book = { id: uid(), title, createdAt: Date.now(), soul: null, chapters: [] };
      d.books.push(book);
...
Only include real weak spots (max 4). If the chapter is strong, say so in overallNote and return an empty weakSpots array — do not invent problems.${soulNote}
...
function attachQuestionsListeners() {
  ...
  if (btn) btn.onclick = async () => {
    ...
    try {
      ...
      const prompt = `You are this author's editor-in-chief. Answer their question directly and honestly, in a few sentences, based on the book below.\n\nQuestion: ${q}\n\nBook chapters:\n${context}`;
