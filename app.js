...
const NAV = [
  ...
  { key: "builder", label: "Book Builder", ic: "🪄" },
...
let ui = {
  ...
  builderTargetWords: 500,
  builderExpandingId: null,
  builderBulkRunning: false,
  builderBulkProgress: null,
};
...
/* ---------------------- BOOK BUILDER (outline -> full draft) ---------------------- */
function ensureOutline(book) {
  if (!book) return [];
  if (!Array.isArray(book.outline)) book.outline = [];
  return book.outline;
}
function newOutlineRow(n) {
  return { id: uid(), title: `Chapter ${n}`, summary: "", chapterId: null };
}
...
/* ---------------------- CLAUDE API ---------------------- */
async function callClaude(prompt, { json = false, maxTokens = 2000 } = {}) {
...
function render() {
  ...
  else if (view === "builder") app.innerHTML = renderBuilder(book);
...
/* ---------------------- BOOK BUILDER ---------------------- */
function renderBuilder(book) {
  let html = `<h1 class="page-title">Book Builder</h1><p class="page-sub">Sketch your chapters as short outlines, then expand each one into a full draft.</p>`;
  if (!book) return html + `<div class="panel empty-state"><p>Open or create a book in the Library first.</p></div>`;

  const outline = ensureOutline(book);

  html += `<div class="panel">
    <p style="font-size:14px;color:var(--text-dim);margin-top:0;">Write a paragraph for each chapter — what happens, who's in it, how it should feel. Author Compass expands each one into full prose in your book's established voice, using your Book Bible for continuity. This creates a first draft to revise, not a finished chapter — everything it writes lands in Chapter Manager afterward, versioned like any other edit.</p>
    <label class="field" style="max-width:220px;">
      <span class="field-label">Target length per chapter</span>
      <input type="number" id="builderTargetWords" min="150" max="3000" step="50" value="${ui.builderTargetWords}" />
    </label>
  </div>`;

  if (!hasKey()) {
    html += `<div class="panel" style="margin-top:18px;"><p class="api-note">Add an Anthropic API key in Settings to expand outlines into drafts (optional — costs a small amount per chapter, only if you choose to add one). You can still write and save your outline paragraphs for free without a key.</p></div>`;
  }

  html += `<div class="panel" style="margin-top:18px;">`;
  if (!outline.length) {
    html += `<p style="color:var(--text-dim);font-size:14px;">No chapter outlines yet.</p>`;
  } else {
    outline.forEach((o, i) => {
      const linkedChapter = o.chapterId ? book.chapters.find((c) => c.id === o.chapterId) : null;
      const isExpanding = ui.builderExpandingId === o.id;
      let statusLine;
      if (isExpanding) statusLine = `<span style="color:var(--gold);">Expanding…</span>`;
      else if (linkedChapter) statusLine = `<span style="color:var(--green);">✓ Expanded — ${wordCount(linkedChapter.content)} words</span> <button class="btn btn-subtle" data-open-drafted="${linkedChapter.id}" style="padding:4px 10px;font-size:12px;margin-left:8px;">Open in Chapter Manager</button>`;
      else statusLine = `<span style="color:var(--text-dim);">Not yet expanded</span>`;

      html += `
        <div class="panel" style="background:rgba(255,255,255,0.02);margin-bottom:14px;">
          <div style="display:flex;justify-content:space-between;gap:10px;align-items:flex-start;flex-wrap:wrap;">
            <label class="field" style="flex:1;min-width:200px;margin-bottom:8px;">
              <span class="field-label">Chapter ${i + 1} title</span>
              <input type="text" data-outline-title="${o.id}" value="${esc(o.title)}" />
            </label>
            <button class="btn btn-subtle" data-remove-outline="${o.id}" style="padding:6px 12px;font-size:12px;margin-top:22px;">✕ Remove</button>
          </div>
          <label class="field">
            <span class="field-label">Outline — what happens in this chapter</span>
            <textarea rows="4" data-outline-summary="${o.id}" placeholder="A paragraph describing the scene, the emotional turn, and what changes by the end…">${esc(o.summary)}</textarea>
          </label>
          <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px;margin-top:6px;">
            <button class="btn btn-primary" data-expand-outline="${o.id}" ${!hasKey() || ui.builderBulkRunning || isExpanding ? "disabled" : ""}>
              ${linkedChapter ? "🪄 Re-expand This Chapter" : "🪄 Expand to Draft"}
            </button>
            <span style="font-size:13px;">${statusLine}</span>
          </div>
        </div>`;
    });
  }
  ...
  html += `
    <button class="btn btn-subtle" data-action="addOutlineRow">+ Add another chapter</button>
  </div>`;
    ...
    html += `
      <div class="panel" style="margin-top:18px;">
        <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px;">
          <div>
            <div style="font-family:var(--serif);font-size:17px;">Expand the whole book</div>
            <div style="color:var(--text-dim);font-size:13px;">Runs through every outline in order, one chapter at a time (this takes a while and uses your API key for each chapter).</div>
          </div>
          <button class="btn btn-primary" data-action="expandAllOutlines" ${!hasKey() || ui.builderBulkRunning ? "disabled" : ""}>
            ${ui.builderBulkRunning ? `Expanding… (${ui.builderBulkProgress ? ui.builderBulkProgress.current + " / " + ui.builderBulkProgress.total : ""})` : "🪄 Expand All Chapters"}
          </button>
        </div>
      </div>`;
  }

  const draftedCount = outline.filter((o) => o.chapterId && book.chapters.find((c) => c.id === o.chapterId)).length;
  if (draftedCount) {
    html += `
      <div class="panel" style="margin-top:18px;">
        <div style="font-family:var(--serif);font-size:17px;margin-bottom:6px;">Download</div>
        <p style="color:var(--text-dim);font-size:13px;margin-top:0;">All expanded chapters, in outline order, as a plain text file — open it in Word, Google Docs, or anywhere else to keep working.</p>
        <button class="btn btn-primary" data-action="downloadManuscript">⬇ Download Full Manuscript (.txt)</button>
      </div>`;
  }
...
function attachViewListeners() {
  ...
  if (view === "builder") attachBuilderListeners();
...
function attachLibraryListeners() {
  ...
  if (form) form.onsubmit = (e) => {
    ...
    if (!title) return;
    setData((d) => {
      const book = { id: uid(), title, createdAt: Date.now(), soul: null, chapters: [], bible: defaultBible(), outline: [] };
      d.books.push(book);
...
/* ---------------------- BOOK BUILDER LOGIC ---------------------- */
async function expandOutlineChapterCore(outlineId) {
  const book = activeBook();
  const outline = ensureOutline(book);
  const o = outline.find((x) => x.id === outlineId);
  if (!o || !o.summary.trim()) return false;

  const idx = outline.findIndex((x) => x.id === outlineId);
  const prevRow = idx > 0 ? outline[idx - 1] : null;
  const prevChapter = prevRow && prevRow.chapterId ? book.chapters.find((c) => c.id === prevRow.chapterId) : null;
  const continuityNote = prevChapter && prevChapter.content
    ? `\n\nThe previous chapter ("${prevChapter.title}") ends with:\n"${prevChapter.content.slice(-500).trim()}"\nContinue naturally from this — don't repeat it, don't restart the scene.`
    : "";

  const bibleNote = bibleContextString(book);
  const soulNote = book.soul ? `\n\nThe soul of this book (preserve this identity): ${book.soul}` : "";
  const target = ui.builderTargetWords || 500;

  const prompt = `You are ghostwriting a full chapter for this author, in the voice and themes already established for this book — not inventing a different style. Expand the outline below into approximately ${target} words of complete, polished narrative prose: real scenes, sensory detail, dialogue where the outline implies it, emotional interiority. Stay faithful to what the outline actually describes — don't invent major plot points it doesn't mention, and don't summarize; write it as a reader would experience it.${bibleNote}${soulNote}${continuityNote}

Chapter title: ${o.title}
Chapter outline: ${o.summary}

Write the full chapter now. Return ONLY the chapter's prose — no title heading, no notes, no meta-commentary, no markdown formatting.`;

  try {
    const maxTokens = Math.min(4000, Math.max(1200, Math.round(target * 2.2)));
    const draft = await callClaude(prompt, { maxTokens });

    let chapter = o.chapterId ? book.chapters.find((c) => c.id === o.chapterId) : null;
    if (!chapter) {
      chapter = { id: uid(), title: o.title, status: "Draft", frozen: false, content: "", health: null, versions: [] };
      book.chapters.push(chapter);
      o.chapterId = chapter.id;
    }
    chapter.title = o.title || chapter.title;
    chapter.content = draft.trim();
    chapter.health = scoreChapterHeuristic(chapter.content);
    const versionNum = (chapter.versions[chapter.versions.length - 1]?.versionNum || 0) + 1;
    chapter.versions.push({
      versionNum, date: new Date().toISOString().slice(0, 10),
      summary: "AI-expanded from Book Builder outline", reason: `Target ~${target} words`, content: chapter.content,
    });
    if (chapter.status === "Draft") chapter.status = "Editing";
    persist();
    return true;
  } catch (e) {
    return false;
...
async function expandOutlineChapter(outlineId) {
  ui.builderExpandingId = outlineId; render();
  const ok = await expandOutlineChapterCore(outlineId);
  ui.builderExpandingId = null; render();
  if (!ok) alert("Couldn't expand this chapter — check your API key in Settings, or try again.");
}

async function expandAllOutlines() {
  const book = activeBook();
  const rows = ensureOutline(book).filter((o) => o.summary.trim());
  if (!rows.length) { alert("Add at least one outline with a paragraph first."); return; }

  ui.builderBulkRunning = true;
  ui.builderBulkProgress = { current: 0, total: rows.length };
  render();

  const failed = [];
  for (let i = 0; i < rows.length; i++) {
    ui.builderBulkProgress = { current: i + 1, total: rows.length };
    ui.builderExpandingId = rows[i].id;
    render();
    const ok = await expandOutlineChapterCore(rows[i].id);
    if (!ok) failed.push(rows[i].title);
  }

  ui.builderBulkRunning = false;
  ui.builderBulkProgress = null;
  ui.builderExpandingId = null;
  render();
  if (failed.length) alert(`Finished, but these chapters failed to expand (check your API key or try them individually): ${failed.join(", ")}`);
}
...
function downloadManuscript() {
  ...
  if (!book) return;
  const outline = ensureOutline(book);
  const ordered = outline.map((o) => (o.chapterId ? book.chapters.find((c) => c.id === o.chapterId) : null)).filter(Boolean);
  const rest = book.chapters.filter((c) => !ordered.includes(c));
  ...
  a.href = url;
  a.download = `${(book.title || "manuscript").replace(/[^a-z0-9]+/gi, "-").toLowerCase()}.txt`;
  document.body.appendChild(a); a.click(); document.body.removeChild(a);
  URL.revokeObjectURL(url);
}

const outlineDebounceTimers = {};
function attachBuilderListeners() {
  const wordsInput = document.getElementById("builderTargetWords");
  if (wordsInput) wordsInput.onchange = () => setUi({ builderTargetWords: Math.max(150, Math.min(3000, Number(wordsInput.value) || 500)) });

  const addBtn = document.querySelector('[data-action="addOutlineRow"]');
  if (addBtn) addBtn.onclick = () => setData((d) => {
    const book = d.books.find((b) => b.id === d.activeBookId);
    const outline = ensureOutline(book);
    outline.push(newOutlineRow(outline.length + 1));
  });

  document.querySelectorAll("[data-remove-outline]").forEach((el) => el.onclick = () => {
    if (!confirm("Remove this outline entry? (Its expanded chapter, if any, stays in Chapter Manager.)")) return;
    setData((d) => {
      const book = d.books.find((b) => b.id === d.activeBookId);
      book.outline = ensureOutline(book).filter((o) => o.id !== el.dataset.removeOutline);
    });
  });

  document.querySelectorAll("[data-outline-title]").forEach((el) => {
    el.oninput = () => {
      const id = el.dataset.outlineTitle, value = el.value;
      clearTimeout(outlineDebounceTimers[id + ":title"]);
      outlineDebounceTimers[id + ":title"] = setTimeout(() => {
        const book = activeBook();
        const o = ensureOutline(book).find((x) => x.id === id);
        if (o) { o.title = value; persist(); }
      }, 500);
    };
  });

  document.querySelectorAll("[data-outline-summary]").forEach((el) => {
    el.oninput = () => {
      const id = el.dataset.outlineSummary, value = el.value;
      clearTimeout(outlineDebounceTimers[id + ":summary"]);
      outlineDebounceTimers[id + ":summary"] = setTimeout(() => {
        const book = activeBook();
        const o = ensureOutline(book).find((x) => x.id === id);
        if (o) { o.summary = value; persist(); }
      }, 500);
    };
  });

  document.querySelectorAll("[data-expand-outline]").forEach((el) => el.onclick = () => expandOutlineChapter(el.dataset.expandOutline));

  const bulkBtn = document.querySelector('[data-action="expandAllOutlines"]');
  if (bulkBtn) bulkBtn.onclick = expandAllOutlines;

  document.querySelectorAll("[data-open-drafted]").forEach((el) => el.onclick = () => {
    ui.selectedChapterId = el.dataset.openDrafted; view = "chapters"; ui.chapterTab = "edit"; render();
  });

  const dlBtn = document.querySelector('[data-action="downloadManuscript"]');
  if (dlBtn) dlBtn.onclick = downloadManuscript;
}
